...
```{r}
N.princ <- nrow(esportes[PRINCIPAL == 1])
N.sec <- nrow(esportes[PRINCIPAL == 0])
```
...
```{r}
N.sec.fut <- nrow(esportes[ID %in% ids.futebol & PRINCIPAL == 0])
N.sec.cor <- nrow(esportes[ID %in% ids.corrida & PRINCIPAL == 0])
```
